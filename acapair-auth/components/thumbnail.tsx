...
export const Thumbnail = ({
  ...
  if (!src) {
    content = (
      <div className="flex h-full w-full flex-col items-center justify-center gap-y-4 rounded-md bg-background bg-slate-300 transition-transform group-hover:-translate-y-2 group-hover:translate-x-2">
        <UserAvatar
          size="lg"
          showBadge
          username={username}
  ...
  return (
    <div className="group relative aspect-video cursor-pointer rounded-md">
      <div className="absolute inset-0 flex items-center justify-center rounded-md bg-[#2d4b4f] opacity-0 transition-opacity group-hover:opacity-100" />
      {content}
      {isLive && src && (
